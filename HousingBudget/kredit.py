...
if __name__ == "__main__":
    ...
    print('Gesamtzinsen: %s EUR: Gesamttilgung: %s EUR' % (tilgungsplan['Zinsen'].sum(),tilgungsplan['Tilgung'].sum()))
    print('#=========================================#')
    ...
    liste_zinsen = [z for z in tilgungsplan['Zinsen']]
    liste_tilgung = [t for t in tilgungsplan['Tilgung']]
    ...
    plt.axvline(x=sollzinsbindung,color='blue', lw=6, alpha=0.5, label='Ende der Sollzinsbindung')
    ...
    plt.xlim(1,liste_monate[-1])
    #plt.ylim(0,max(liste_tilgung[0], liste_zinsen[0]))
    plt.xticks(fontsize=13)
    plt.yticks(fontsize=13)
    ...
    ax.plot(liste_monate, liste_zinsen, label='Zinsen')
    ax.plot(liste_monate, liste_tilgung, label='Tilgung')
